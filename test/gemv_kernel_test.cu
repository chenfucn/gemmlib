 ...
 *
 * @file gemv_kernel_test.cu
 * 
 * Test the mixed precision gemv kernel specifically for token gen
 * with sliced k.
 */
...
#include "gemm/kernel/quant_b4_gemv.h"
...
class QuantB4GemmTestDevKernel {
 public:
  ...
  using TestKernel = mickey::gemm::kernel::QuantB4Gemv<QuantBlocking, false, kSplitK, kStages>;
...
template <typename QuantBlocking, int kSplitK, int kStages>
void test_quantb4_gemv(int n, int k) {
  std::cout << "Testing Blocking: " << QuantBlocking::kRow << "x" << QuantBlocking::kColumn 
            << ", kSplitK: " << kSplitK << ", kStages: " << kStages;
  std::cout << ", m: " << 1 << ", n: " << n << ", k: " << k << std::endl;

  using Test = QuantB4GemmTestDevKernel<QuantBlocking, kSplitK, kStages>;
  Test test;
  cutlass::gemm::GemmCoord problem_size(1, n, k);
  ...
  cutlass::HostTensor<cutlass::half_t, cutlass::layout::RowMajor> tensor_a({1, k});
  cutlass::reference::host::TensorFillRandomUniform(tensor_a.host_view(), 174321, 1.5f, -1.125f, 5);

  cutlass::HostTensor<cutlass::half_t, cutlass::layout::RowMajor> tensor_b({k, n});
  cutlass::reference::host::TensorFillRandomUniform(tensor_b.host_view(), 193456, 1.75f, -1.25f, 5);
...
TEST(QuantB4Gemv, PackedBTest) {

  test_quantb4_gemv<cutlass::MatrixShape<1, 16>, 4, 3>(48, 64 * 29 - 16);
  test_quantb4_gemv<cutlass::MatrixShape<32, 1>, 4, 3>(64, 64 * 27 + 32);
