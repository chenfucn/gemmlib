 ...
 *
 * @file tb_quantb_tile_loader_tests.cu
 */
...
#include "gemm/threadblock/quantb_tile_loader.h"
#include "gemm/threadblock/quantb_meta_loader.h"
...
>
struct PackedBLoaderTestKernel {
 public:
  //
  // Type definitions
  //
  ...
  static_assert(TBShape::kK % 32 == 0); // int4 unit tile is 16x16

  using WarpBShape = cutlass::gemm::GemmShape<1, 64, TBShape::kK>;  // TODO: TEST kK = 32
  static_assert(TBShape::kN % WarpBShape::kN == 0);
  static_assert(TBShape::kK % WarpBShape::kK == 0);
  ...
  using PackedBLoader = mickey::gemm::threadblock::QuantBTileLoader<TBShape, kThreadCount>;
  using MetaLoader = mickey::gemm::threadblock::QuantBScaleLoader<QuantBlocking, TBShape, WarpBShape, ElementT, kThreadCount, false>;
  ...
  static cutlass::Status can_implement(const Params &params) {
    if ((params.problem_size_.k() % QuantBlocking::kRow != 0) ||
      ...
      std::cerr << "PackedBLoaderTestKernel validation fail: partial quantization block not supported!" << std::endl;
      return cutlass::Status::kErrorInvalidProblem;
    }
    if (reinterpret_cast<uintptr_t>(params.ptr_packed_b_) % 128) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.ptr_packed_b_ is not aligned to 128 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.b_byte_stride_ % 128) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.b_byte_stride_ is not aligned to 128 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (reinterpret_cast<uintptr_t>(params.ptr_scales_) % 16) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.ptr_scales_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.scales_byte_stride_ % 16) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.scales_byte_stride_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if constexpr (has_quant_offset) {
      if (params.ptr_offsets_ == nullptr || params.offsets_byte_stride_ == 0) {
        std::cerr << "PackedBLoaderTestKernel validation fail: Required quantization offsets are not provided!" << std::endl;
        return cutlass::Status::kErrorInvalidProblem;
      }
      if (reinterpret_cast<uintptr_t>(params.ptr_offsets_) % 16) {
        std::cerr << "PackedBLoaderTestKernel validation fail: params.ptr_offsets_ is not aligned to 16 bytes!" << std::endl;
        return cutlass::Status::kErrorMisalignedOperand;
      }
      if (params.offsets_byte_stride_ % 16) {
        std::cerr << "PackedBLoaderTestKernel validation fail: params.offsets_byte_stride_ is not aligned to 16 bytes!" << std::endl;
        return cutlass::Status::kErrorMisalignedOperand;
      }
    } else {
      if (params.ptr_offsets_ != nullptr || params.offsets_byte_stride_ != 0) {
        std::cerr << "PackedBLoaderTestKernel validation fail: quantization offsets are provided to scale only kernel!" << std::endl;
        return cutlass::Status::kErrorInvalidProblem;
      }
    }

    if (reinterpret_cast<uintptr_t>(params.ptr_output_) % 16) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.ptr_output_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.output_byte_stride_ % 16) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.output_byte_stride_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.problem_size_.k() % 16 != 0) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.problem_size_.k() is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorInvalidProblem;
    }
    if (params.problem_size_.k() > params.b_byte_stride_) {
      std::cerr << "PackedBLoaderTestKernel validation fail: params.problem_size_.k() is greater than params.b_byte_stride_!" << std::endl;
    ...
    if constexpr (kSplitK > 1){
      if (params.gemm_k_size_ < TBShape::kK * kStages * 2) {
        ...
        std::cerr << "PackedBLoaderTestKernel validation fail: kSplitK is too small, k: " << params.gemm_k_size_ << " is smaller than " << (TBShape::kK * kStages * 4) << std::endl;
  ...
  void operator()(Params const &params, SharedStorage &shared_storage) {
    ...
    const int n_start = blockIdx.y * TBShape::kN;
    const int n_end = min(params.problem_size_.n(), (blockIdx.y + 1) * TBShape::kN);
    ...
    if constexpr (kDebugPrint) {
      if (threadIdx.x == 0) {
        printf("k_start %d, k_end %d, n_start %d, n_end %d\n",
    ...
    ElementT* shared_scale_ptr = shared_storage.shared_Scale.data();
    ...
    for (; smem_write_stage < kStages - 1; ++smem_write_stage, load_k += TBShape::kK) {
      ...
      meta_loader.load_to_smem(threadIdx.x, load_k, min(k_end - load_k, TBShape::kK), scale_smem_ptr);
      packed_b_loader.load_to_smem(packed_b_smem_ptr, threadIdx.x);
      ++packed_b_loader;
    ...
    for (; proc_k < k_end; smem_write_stage = (smem_write_stage + 1) % kStages, smem_read_stage = (smem_read_stage + 1) % kStages, proc_k += TBShape::kK, load_k += TBShape::kK){
      ...
      meta_loader.load_to_smem(threadIdx.x, load_k, min(k_end - load_k, TBShape::kK), scale_smem_write_ptr);
      MetaLoader::load_fragment(lane_idx, scale_smem_read_ptr, warp_n_idx * WarpBShape::kN, warp_k_idx * WarpBShape::kK, fragment_scales);

      meta_loader.process(fragment_scales, fragment_addon);

      packed_b_loader.load_to_smem(packed_b_smem_write_ptr, threadIdx.x);
      ++packed_b_loader;
      ...
      for (int mma_iter = 0; mma_iter < kWarpMmaIterations; ++mma_iter)  {
        static_assert(WarpBShape::kN == 64);
        PackedBLoader::load_fragment_tile4(threadIdx.x, packed_b_smem_read_ptr, warp_n_idx, warp_k_idx * kWarpMmaIterations + mma_iter, fragment_packed_b);
        ...
        for (int b_tile_n = 0; b_tile_n < (WarpBShape::kN/8); ++b_tile_n) {
          int n = n_start + warp_n_idx * WarpBShape::kN + b_tile_n * 8 + (lane_idx / 4);
          int k = proc_k + warp_k_idx * WarpBShape::kK + mma_iter * InstructionShape::kK + (lane_idx % 4) * 2;
          if (n < n_end && k < k_end) {
...
class TBDequantBTest {
 public:
  ...
  using TestKernel = PackedBLoaderTestKernel<QuantBlocking, false, TBShape, kSplitK, kStages>;
...
void test_tb_dequant(int m, int n, int k) {
  ...
  using Test = TBDequantBTest<QuantBlocking, TBShape, kSplitK, kStages>;
  Test test;
  ...
  cutlass::reference::host::TensorFillRandomUniform(tensor_b.host_view(), 1753, -1.5f, 1.75f);
  ...
  if (!passed) {
    std::cerr << "Mismatch found in test_tb_dequant!" << std::endl;
...
TEST(TBDequant, Packed4BTest) {
  // Smaller versions for debugging
  // test_tb_dequant<cutlass::MatrixShape<32, 1>, cutlass::gemm::GemmShape<1, 128, 32>, 1, 3>(1, 128, 64);
  // test_tb_dequant<cutlass::MatrixShape<1, 32>, cutlass::gemm::GemmShape<1, 64, 64>, 1, 3>(1, 64, 64);

  test_tb_dequant<cutlass::MatrixShape<32, 1>, cutlass::gemm::GemmShape<1, 256, 64>, 4, 3>(1, 512 - 32, 64 * 20 + 32);
  test_tb_dequant<cutlass::MatrixShape<1, 32>, cutlass::gemm::GemmShape<1, 128, 64>, 4, 3>(1, 128 + 32, 64 * 22 - 32);
  test_tb_dequant<cutlass::MatrixShape<1, 64>, cutlass::gemm::GemmShape<1, 256, 64>, 4, 3>(1, 512 - 64, 64 * 20 + 16);
  test_tb_dequant<cutlass::MatrixShape<16, 1>, cutlass::gemm::GemmShape<1, 128, 64>, 4, 3>(1, 512 + 32, 64 * 22 - 32);
}
