...
struct QuantBScaleLoader<cutlass::MatrixShape<block_size_, 1>, WarpShape_, ElementT_, DebugPrint> {
  ...
  void load_to_smem(const int lane_idx, const int start_k, const int k_cnt, ElementT* smem) const {
    ...
    }
    {
      ...
      asm volatile(
          ...
          "}\n"
          ::"r"((int)((kScaleLoadsPerWarp % 32) > lane_idx)),
            "r"(smem_int_ptr),
  ...
  CUTLASS_DEVICE
  void load_fragment(const int lane_idx, FragmentScales &frag_scales, const ElementT* smem) const {
    const ElementT* scales_ptr = smem + div_power2<4>(lane_idx);
