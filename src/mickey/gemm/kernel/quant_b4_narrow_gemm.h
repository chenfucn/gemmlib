...
template<typename ElementT, typename Shape, typename QuantBlocking, int Stages>
...
struct QuantB4NarrowGemm {
 public:
  ...
  static_assert(WarpShape::kK % 32 == 0,
    "K stride too small leading to inefficient main loop!");
  ...
  using ATileLoader = mickey::gemm::warp::SwizzleTileLoader<WarpShape::kM, WarpShape::kK * kElementSize, 32>;
  ...
  static int const kThreads = 32 * kWarps;

  using MainLoopSharedBuffer = MmaLoopSharedBuffer<ElementT, WarpShape, QuantBlocking, kStages>;
  static_assert(MainLoopSharedBuffer::kPackedBSizePerIter == PackedBLoader::kByteSize);
  ...
  static cutlass::Status can_implement(const Params &params) {
    if ((reinterpret_cast<uintptr_t>(params.ptr_a_) % 16)) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.ptr_a_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.a_byte_stride_ % 16) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.a_byte_stride_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if ((params.problem_size_.k() % QuantBlocking::kRow != 0) ||
        (params.problem_size_.n() % QuantBlocking::kColumn) != 0){
      std::cerr << "QuantB4NarrowGemm validation fail: partial quantization block not supported!" << std::endl;
      return cutlass::Status::kErrorInvalidProblem;
    }
    if (reinterpret_cast<uintptr_t>(params.ptr_packed_b_) % 128) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.ptr_packed_b_ is not aligned to 128 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.b_byte_stride_ % 128) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.b_byte_stride_ is not aligned to 128 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (reinterpret_cast<uintptr_t>(params.ptr_scales_) % 16) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.ptr_scales_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.scales_byte_stride_ % 16) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.scales_byte_stride_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if constexpr (has_quant_offset) {
      if (params.ptr_offsets_ == nullptr || params.offsets_byte_stride_ == 0) {
        std::cerr << "QuantB4NarrowGemm validation fail: Required quantization offsets are not provided!" << std::endl;
        return cutlass::Status::kErrorInvalidProblem;
      }
      if (reinterpret_cast<uintptr_t>(params.ptr_offsets_) % 16) {
        std::cerr << "QuantB4NarrowGemm validation fail: params.ptr_offsets_ is not aligned to 16 bytes!" << std::endl;
        return cutlass::Status::kErrorMisalignedOperand;
      }
      if (params.offsets_byte_stride_ % 16) {
        std::cerr << "QuantB4NarrowGemm validation fail: params.offsets_byte_stride_ is not aligned to 16 bytes!" << std::endl;
        return cutlass::Status::kErrorMisalignedOperand;
      }
    } else {
      if (params.ptr_offsets_ != nullptr || params.offsets_byte_stride_ != 0) {
        std::cerr << "QuantB4NarrowGemm validation fail: quantization offsets are provided to scale only kernel!" << std::endl;
        return cutlass::Status::kErrorInvalidProblem;
      }
    }

    if (reinterpret_cast<uintptr_t>(params.ptr_output_) % 16) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.ptr_output_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.output_byte_stride_ % 16) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.output_byte_stride_ is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorMisalignedOperand;
    }
    if (params.problem_size_.n() > (params.output_byte_stride_ / kElementSize)) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.problem_size_.n() is greater than params.output_byte_stride_!" << std::endl;
      return cutlass::Status::kErrorInvalidProblem;
    }
    if (params.problem_size_.k() % 16 != 0) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.problem_size_.k() is not aligned to 16 bytes!" << std::endl;
      return cutlass::Status::kErrorInvalidProblem;
    }
    if (params.problem_size_.k() > (params.b_byte_stride_ / 8)) {
      std::cerr << "QuantB4NarrowGemm validation fail: params.problem_size_.k() is greater than params.b_byte_stride_!" << std::endl;
    ...
    if constexpr (kSplitK > 1){
      if (params.gemm_k_size_ < WarpShape::kK * kStages) {
        ...
        std::cerr << "QuantB4NarrowGemm validation fail: kSplitK is too small, k: " << params.gemm_k_size_ << " is smaller than " << (WarpShape::kK * kStages) << std::endl;
  ...
  void operator()(Params const &params, SharedStorage &shared_storage) {
    ...
    uint8_t* packed_b_shared_ptr = packed_b_loader.get_smem_lane_ptr(
        shared_storage.smem[warp_idx].main_loop.shared_B.data(), lane_idx);
    ...
    PackedBLoader::load_to_register(lane_idx, 0, packed_b_smem_read_ptr, fragment_packed_b[0]);
    ...
    while (load_k < k_end + (kStages - 1) * WarpShape::kK){
      ...
      for (int iter2 = 0; iter2 < kMmaIterations * 2; ++iter2) {
        ...
        if ((next_iter2 % kMmaIterPerPackedB) == 0) {
          ...
          PackedBLoader::load_to_register(lane_idx, next_iter, packed_b_smem_read_ptr, fragment_packed_b[(next_iter2 / kMmaIterPerPackedB) % 2]);
        }
        ...
        meta_loader.dequant_k16(iter2 % kMmaIterations, fragment_packed_b[(iter2 / kMmaIterPerPackedB) % 2], fragment_scales[(iter2 / kMmaIterations) % 2], fragment_addon, fragment_b);
