 ...
 *
 * @file threadblock/quantb_meta_loader.h
...
template <
    ...
    typename ThreadblockShape_,  ///! Shape of the threadblock tile (concept: GemmShape, kM ignored)
    typename WarpShape_,         ///! Shape of the warp tile (concept: GemmShape, kM ignored)
    typename ElementT_,          ///! Data type of the scales and dequantized B
    unsigned NumThreads_,        ///! Number of threads
...
template <
    ...
    bool DebugPrint>
struct QuantBScaleLoader<cutlass::MatrixShape<block_size_, 1>, ThreadblockShape_, WarpShape_, ElementT_, NumThreads_, DebugPrint> {
  //
  // Type definitions
  //
  ...
  static_assert((ThreadblockShape::kN % WarpShape::kN) == 0 && (ThreadblockShape::kK % WarpShape::kK) == 0,
                "Threadblock shape must be multiple of warp shape");
  ...
  static constexpr unsigned kAccessSize = 16;  // cp.async 16 bytes at a time
  ...
  static constexpr int kSmemSize = ThreadblockShape::kN * div_up(ThreadblockShape::kK, QuantBlocking::kRow);
  static constexpr int kScaleLoads = (kSmemSize * sizeof(ElementT)) / kAccessSize;
  ...
  CUTLASS_DEVICE
  void load_to_smem(const unsigned thread_id, const int start_k, const int k_cnt, ElementT* smem) const {
    constexpr unsigned load_stride = (kThreads * kAccessSize / sizeof(ElementT));
    int lane_ptr_offset = mul_power2<kAccessSize / sizeof(ElementT)>(thread_id);
    ...
    // Load scales to smem
    CUTLASS_PRAGMA_UNROLL
    for (int i = 0; i < (kScaleLoads / kThreads); ++i, lane_ptr_offset += load_stride) {
      const int k_idx = div_power2<ThreadblockShape::kN>(lane_ptr_offset);
      const int n_idx = mod_power2<ThreadblockShape::kN>(lane_ptr_offset);
    ...
    if constexpr ((kScaleLoads % kThreads) > 0){
      const int k_idx = div_power2<ThreadblockShape::kN>(lane_ptr_offset);
      const int n_idx = mod_power2<ThreadblockShape::kN>(lane_ptr_offset);

      unsigned smem_int_ptr = cutlass::arch::cutlass_get_smem_pointer(&smem[lane_ptr_offset]);
      int src_in_bytes = ((k_idx < k_loads && n_idx < n_cnt) ? kAccessSize : 0);
      asm volatile(
          "{\n"
          "  .reg .pred p;\n"
          "  setp.ne.b32 p, %0, 0;\n"
          ...
          "}\n"
          ::"r"((int)((kScaleLoads % kThreads) > thread_id)),
            "r"(smem_int_ptr),
            "l"(&scales_ptr[k_idx * scales_stride + n_idx]),
            "n"(16), "r"(src_in_bytes));
    }
  }

  //
  // Warp layout: As dictated by 16b tensor core layout, 32
  ...
  CUTLASS_DEVICE
  static void load_fragment(const int lane_idx, const ElementT* smem, int n_offset, int k_offset, FragmentScales &frag_scales) {
    assert(n_offset < ThreadblockShape::kN);
    assert(n_offset % WarpShape::kN == 0);
    assert(k_offset < ThreadblockShape::kK);
    assert(k_offset % WarpShape::kK == 0);

    const ElementT* scales_ptr = smem + div_power2<QuantBlocking::kRow>(k_offset) * ThreadblockShape::kN + n_offset + div_power2<4>(lane_idx);

    CUTLASS_PRAGMA_UNROLL
    for (int kk = 0; kk < kMetaChunkCount; ++kk) {
      CUTLASS_PRAGMA_UNROLL
      for (int nn = 0; nn < kMetaFragSize; ++nn) {
        frag_scales[kk * kMetaFragSize + nn] = scales_ptr[nn << 3];
      }
      scales_ptr += ThreadblockShape::kN;
    }
  }
  ...
  void dequant_k16(
    ...
    for (int nn = 0; nn < (WarpShape::kN / 16); ++nn) {
      ...
      half2 addon_pair = __half2half2(addon[nn * 2]);
      half2 addon_pair1 = __half2half2(addon[nn * 2 + 1]);
...
template <
    ...
    unsigned NumThreads_,
    bool DebugPrint>
struct QuantBScaleLoader<cutlass::MatrixShape<1, block_size_>, ThreadblockShape_, WarpShape_, ElementT_, NumThreads_, DebugPrint> {
  //
  // Type definitions
  //
  ...
  //
  // HBM -> SMEM, 16 bytes per load, no leftover since WarpShape::kN is multiple of 16
  static constexpr int kSmemSize = ThreadblockShape::kK * div_up(ThreadblockShape::kN, QuantBlocking::kColumn);
  static constexpr int kScaleLoads = (kSmemSize * sizeof(ElementT)) / kAccessSize;
  ...
  void load_to_smem(const unsigned thread_id, const int start_k, const int k_cnt, ElementT* smem) const {
    ...
    CUTLASS_PRAGMA_UNROLL
    for (int i = 0; i < (kScaleLoads / kThreads); ++i, lane_ptr_offset += load_stride) {
      const int k_idx = lane_ptr_offset % ThreadblockShape::kK;
      const int n_idx = lane_ptr_offset / ThreadblockShape::kK;
      cutlass::arch::cp_async_zfill<16, cutlass::arch::CacheOperation::Global>(
          &smem[lane_ptr_offset],
    ...
    }
    {
      ...
      asm volatile(
          ...
          ::"r"((int)((kScaleLoads % kThreads) > thread_id)),
            "r"(smem_int_ptr),
            "l"(&scales_ptr[n_idx * scales_stride + k_idx]),
            "n"(16), "r"(src_in_bytes));
    }
  }
  ...
  static void load_fragment(const int lane_idx, const ElementT* smem, int n_offset, int k_offset, FragmentScales &frag_scales) {
    ...
    // T2        T3
    // T3        T3
    smem += k_offset + (n_offset / QuantBlocking::kColumn) * ThreadblockShape::kK + (mod_power2<4>(lane_idx) << 2);

    CUTLASS_PRAGMA_UNROLL
    for (int nn = 0; nn < kMetaChunkCount; ++nn) {
      const uint32_t* scales_ptr = reinterpret_cast<const uint32_t*>(smem);
      CUTLASS_PRAGMA_UNROLL
      for (int kk = 0; kk < kMetaFragSize; kk += 4) {
        uint32_t* frag_ptr = reinterpret_cast<uint32_t *>(frag_scales.data() + nn * kMetaFragSize + kk);
        frag_ptr[0] = scales_ptr[0];
        frag_ptr[1] = scales_ptr[1];
        scales_ptr += 8;
      }
      smem += ThreadblockShape::kK;
    }
  }
  ...
  void dequant_k16(
    ...
    for (int nn = 0; nn < (WarpShape::kN / 16); ++nn) {
      ...
      half2 const* const addon_pair = reinterpret_cast<half2 const*>(frag_addon.data() + idx); // k_offset / 16 * 4
      ...
      if constexpr (DebugPrint) {
        const int lane_id = threadIdx.x % 32;
        const char* const format = (lane_id == 31) ? "%f=%fx%f, %f=%fx%f\n\n" 
                                 : ((lane_id % 4) == 3) ? "%f=%fx%f, %f=%fx%f\n" : "%f=%fx%f, %f=%fx%f, ";
